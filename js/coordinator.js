...
function orderQryBySalesMan(){
    ...
    $.ajax({
        url:'http://132.226.250.48:8083/api/order/zona/'+ user.zone,
        type:'GET',
        datatype:'JSON',
        success:function(respuesta){
            ...
            tableList+="<tr>";
            tableList+="<th> Asesor Comercial </th>";
            tableList+="<th> Id </th>";
            tableList+="<th> Fecha </th>";
            tableList+="<th> Estado </th>";
            tableList+="<th> Acciones </th> </tr>";

            for(i=0;i<respuesta.length;i++){
                tableList+="<tr>";
                tableList+="<td style='width: 200px'>" + respuesta[i].salesMan.name + "</td>";
                ...
                tableList+="<td> <button class='btn btn-secondary' onclick='orderDet(" + JSON.stringify(respuesta[i]) + ")'>Detalle</button>";
                tableList+="</tr>";
            }
            tableList+="</table>";
            tableList+="</div>";
...
function orderDet(respuesta) {
    
    sessionStorage.setItem('order', JSON.stringify(respuesta));
